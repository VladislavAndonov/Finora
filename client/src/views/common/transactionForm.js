...
export const transactionForm = ({
    ...
    const cardColor = selectedCategory?.color ?? "rgb(163, 163, 165)";

    return html`
    <div class="transaction-form">
 
        <nav class="transaction-form__header">
            <button class="transaction-form__btn" type="button" @click=${() => history.back()}>
                <i class="ph-bold ph-arrow-left"></i>
            </button>
            <h1 class="transaction-form__title">${title}</h1>
            ${onDelete ? html`
                <button
                    class="transaction-form__btn transaction-form__btn--delete"
                    type="button"
                    @click=${onDelete}
            ...
            ` : html`<div class="transaction-form__spacer"></div>`}
        </nav>
        ...
        <form class="transaction-form__body" @submit=${onSubmit}>
                ...
                <div class="transaction-form__card-body">
                    <div class="transaction-form__card-amount-wrap">
                        <div class="transaction-form__currency-row">
                            ...
                            <input
                                class="transaction-form__amount-input"
                                type="number"
                                name="amount"
                                inputmode="numeric"
                                step="any"
            ...
            ? (transaction.amount % 1 === 0 ? transaction.amount : Number(transaction.amount).toFixed(2))
            : ""}
                            >
                        </div>
            ...
            <div class="input-wrapper">
                ...
                <textarea
                    class="transaction-form__input transaction-form__input--textarea"
                    name="note"
                    maxlength="200"
                    placeholder="Note"
            ...
            <div class="transaction-form__datetime-row"
                ...
                <input
                    class="transaction-form__datetime--hidden"
                    type="datetime-local"
                    id="date"
                    name="date"
            ...
            <div class="transaction-form__actions">
                <button
                    class="transaction-form__save-btn"
                    type="submit"
                    ?disabled=${state.isSubmitting}
                >
